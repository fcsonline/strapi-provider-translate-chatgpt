...
function parseLocale(strapiLocale, localeMap = {}, direction = 'target') {
    ...
    (0, lodash_1.defaults)(localeMap, defaultLocaleMap);
    ...
    switch (stripped) {
        ...
        case 'PT':
            if (unstripped === 'PT-PT')
                possiblyUnstrippedResult = unstripped;
            else if (unstripped === 'PT-BR')
                possiblyUnstrippedResult = unstripped;
            else
                possiblyUnstrippedResult = localeMap[stripped] || 'PT-PT';
            break;
        case 'EN':
            if (unstripped === 'EN-GB')
                possiblyUnstrippedResult = unstripped;
            else if (unstripped === 'EN-US')
                possiblyUnstrippedResult = unstripped;
            else
                possiblyUnstrippedResult = localeMap[stripped] || 'EN-US';
            break;
        default:
            if (localeMap[stripped])
                possiblyUnstrippedResult = localeMap[stripped];
